...
for( i in 1:numIterations )
{
  ...
  # metro post
	pOld <- dbinom( 583, 417, piOld) * (dexp(piOld, rate=5)/0.9932621)
	...
	pNew <- dbinom( 583, 417, piNew) * (dexp(piNew, rate=5)/0.9932621)
...
for( x in xVals )
{
	posteriorDistGrid[i] <- (dexp(x, rate=5)/0.9932621) * dbinom( 583, 417, x)
	sum = sum + posteriorDistGrid[i];
	i <- i + 1;	
}

# plot beta 
dbetasum = sum(dbeta(myHist$mids, 40+10, 40+583))
lines( myHist$mids, dbeta(myHist$mids, 40+583, 40+10)/dbetasum, col="red")
